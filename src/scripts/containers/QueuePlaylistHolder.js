...
const mapStateToProps = (state, ownProps) => {
  const {[ownProps.type]: holder, playlist, songs, user, duration} =
    getWithNestedEntities[ownProps.type](state, ownProps.id);
...
class QueuePlaylistHolder extends Component {
  ...
  componentWillReceiveProps = nextProps =>
    nextProps.playlist.songs.length ? false : this.props.remove();
    ...
    return (
      <div className={`${styles.root} ${this.state.opened ? styles.opened : ''}`}>
        <div style={{cursor: 'pointer'}} onClick={this.toggleSongList}>
          ...
          <div className={styles.info}>
            <div className={styles.title}>
              ...
              <i className={styles.openInNewIcon}
                onClick={openStreamInNewTab(this.props.holder.originId)}>open_in_new</i>
            </div>
            ...
            <div className={styles.duration}>
              ...
              <i className={styles[this.state.opened ? 'openedIcon' : 'closedIcon']}>
                arrow_drop_down</i>
            </div>
          </div>
        ...
        <div className={`${styles.songList} ${this.state.opened ? styles.visible : ''}`}>
          {this.props.playlist.songs.map(songId =>
            <QueuePlaylistHolderSong playlist={this.props.playlist} id={songId} key={songId} />)}
        </div>
      </div>
    );
  }
