...
export const search = (filters, token) => luch.post(withStreamsUrl('/search'), removeUndefinedAttrs({term: filters.term, token})).then(getJson);
...
export const create = (playlist_title, playlist_description, tags, default_artwork_url, songs, token) => luch(
  withStreamsUrl(''),
  {
    method: 'POST',
...
export const getComments = (id, limit, offset) => luch.get(withStreamUrl(id, '/comments'), {limit, offset}).then(getJson);
export const addComment = (id, body, token) => luch.post(withStreamUrl(id, '/add_comment'), {body, token}).then(getJson);
