...
export const getToken = (email, password) => luch.post(getAbsoluteUrl(`${SIGN_IN_PATH}`), {email, password})
  .then(getJson).then(data => data.token || Promise.reject(data));

export const getCurrentUserData = token => luch.get(getAbsoluteUrl(`/me`), {token})
...
export const networkLogin = (token, network) => luch.post(getAbsoluteUrl('/enter'), {token, network, platform: 'web'})
